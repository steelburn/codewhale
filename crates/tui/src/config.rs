...
// Provider enumeration and helpers live in the `provider` leaf module
// and are re-exported below so every `crate::config::ApiProvider` path
// resolves unchanged (#3949).
mod provider;
use provider::subagent_provider_key_matches;
pub use provider::*;

// Provider capability metadata lives in the `capability` leaf module and is
// re-exported below so existing `crate::config::<item>` paths keep working
// (#3949).
mod capability;
pub use capability::*;
